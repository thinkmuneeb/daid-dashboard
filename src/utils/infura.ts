...
import { DAI, UNI, DAID } from '../constants/tokens'
...
export const getCost = async (amount) => {
  ...
  const [inputAmount, _] = await exchange.methods
    .getAmountsIn(new BigNumber(amount).toFixed(), [DAI.addr, DAID.addr])
    .call()
  return inputAmount
}
...
export const getProceeds = async (amount) => {
  ...
  const [_, outputAmount] = await exchange.methods
    .getAmountsOut(new BigNumber(amount).toFixed(), [DAID.addr, DAI.addr])
    .call()
  return outputAmount
}
