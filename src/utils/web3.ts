...
import { DAID, DAI } from '../constants/tokens'
...
export const buyDAID = async (buyAmount, maxInputAmount) => {
  const account = await checkConnectedAndGetAddress()
...
export const sellDAID = async (sellAmount, minOutputAmount) => {
  const account = await checkConnectedAndGetAddress()
  const router = new web3.eth.Contract(uniswapRouterAbi, UniswapV2Router02)
  const deadline = Math.ceil(Date.now() / 1000) + DEADLINE_FROM_NOW
...
export const addLiquidity = async (amountDAID, amountDAI, slippage) => {
  const account = await checkConnectedAndGetAddress()
  const router = new web3.eth.Contract(uniswapRouterAbi, UniswapV2Router02)
  const deadline = Math.ceil(Date.now() / 1000) + DEADLINE_FROM_NOW
  const slippageBN = new BigNumber(slippage)
  const minAmountDAID = new BigNumber(amountDAID)
  ...
  await router.methods
    .addLiquidity(
      DAID.addr,
      DAI.addr,
      new BigNumber(amountDAID).toFixed(),
...
export const removeLiquidity = async (
  ...
  await router.methods
    .removeLiquidity(
      ...
      new BigNumber(minAmountDAID).toFixed(),
