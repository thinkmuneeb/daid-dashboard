...
import { useTransaction, useUserData, useDaidData } from '../hooks'
...
const DAOModal = () => {
  const { redeemable, coupons } = useDaidData()
  ...
  const tabMap = {
    [Stages.Purchase]: {
      ...
      action: (value: BigNumber) =>
        sendTransaction((cb) =>
          purchaseCoupons(DAIDS.addr, toBaseUnitBN(value, DAID.decimals), cb),
        ),
    },
  ...
  const updatePremium = async (purchaseAmount) => {
    ...
    const purchaseAmountBase = toBaseUnitBN(purchaseAmount, DAID.decimals)
    const premium = await getCouponPremium(DAIDS.addr, purchaseAmountBase)
    const premiumFormatted = toTokenUnitsBN(premium, DAID.decimals)
      ...
      <ModalContentWrap>
        ...
        {!isEnabled ? (
          <BigButton
            ...
            onClick={() =>
              sendTransaction((cb) => approve(DAID.addr, DAIDS.addr, cb))
            }
          >
            Enable
          </BigButton>
