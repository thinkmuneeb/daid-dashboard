...
import { useTransaction, useUserPoolData, useDaidData } from '../hooks'
import { MAX_UINT256 } from '../constants/values'
import { approve, claimPool, providePool } from '../utils/web3'
import { DAI, DAID } from '../constants/tokens'
...
const RewardsModal = () => {
  const { poolAddress, pairBalanceDai, pairBalanceDaid } = useDaidData()
  const {
    daiBalance,
    daiAllowance,
    claimable,
  ...
  const tabMap = {
    [RewardsStages.Claim]: {
      ...
      action: (value: BigNumber) =>
        sendTransaction((cb) =>
          claimPool(poolAddress, toBaseUnitBN(value, DAID.decimals), cb),
        ),
    ...
    [RewardsStages.Provide]: {
      ...
      action: (value: BigNumber) =>
        sendTransaction((cb) =>
          providePool(poolAddress, toBaseUnitBN(value, DAID.decimals), cb),
        ),
      helpText: (
        <React.Fragment>
          Provide earned DAID (with equivalent DAI) to compound bonded UNI-LP.
  ...
  const ratio = pairBalanceDai.isZero()
    ? new BigNumber(1)
    : pairBalanceDai.div(pairBalanceDaid)

  const amountDaid = toBaseUnitBN(formValue, DAID.decimals)
  const newAmountDai = amountDaid
    .multipliedBy(ratio)
      ...
      <ModalColorRush>
        ...
        <ModalContentWrap>
          {txHash ? (
            ...
            <EnableTout copy="To Provide DAID-DAI LP, you need to enable it first." />
          )}
        </ModalContentWrap>
