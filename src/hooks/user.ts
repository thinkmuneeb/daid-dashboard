...
import { DAI, UNI, DAID, DAIDS } from '../constants/tokens'
import {
  ownership as calculateOwnership,
  toTokenUnitsBN,
} from '../utils/number'
import { useDaidData, totalDAIDSAtom } from './daid'
  ...
  useUpdater(
    async function updateUserInfo(isCancelled: boolean) {
      ...
      const [
        ...
        getTokenBalance(DAID.addr, user),
        getTokenAllowance(DAID.addr, user, DAIDS.addr),
        getBalanceOfStaged(DAIDS.addr, user),
        getBalanceBonded(DAIDS.addr, user),
        getStatusOf(DAIDS.addr, user),
        getFluidUntil(DAIDS.addr, user),
        getLockedUntil(DAIDS.addr, user),
        getTokenBalance(DAIDS.addr, user),
        // getBalanceOfCoupons(DAIDS.addr, user),
      ])

      if (!isCancelled) {
        setBalance(toTokenUnitsBN(balanceStr, DAID.decimals))
        setAllowance(new BigNumber(allowanceStr))
        setStaged(toTokenUnitsBN(staged, DAID.decimals))
        setBonded(toTokenUnitsBN(bonded, DAID.decimals))
        setStatus(parseInt(status, 10))
        setFluidUntil(parseInt(fluidUntil, 10))
        setLockedUntil(parseInt(lockedUntil, 10))
        setUserStake(toTokenUnitsBN(userStake, DAIDS.decimals))
      }
    },
    5000,
    [user],
...
const governanceDataSelector = selector({
  ...
  get: ({ get }) => {
    const ownership = calculateOwnership(get(userStakeAtom), get(totalDAIDSAtom))
    return {
      ownership,
